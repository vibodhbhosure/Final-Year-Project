...
export function ApexChartAmbient(props) {
  const [dataPoints, setDataPoints] = useState([
    { value: 0, timestamp: new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) }, // Initial data point with IST timestamp
    ...
    const subscription = PubSub.subscribe('device/12/data').subscribe({
      next: data => {
        try {
          ...
          const humidityValue = parseFloat(sensorData.ambienttemperature);
          if (!isNaN(humidityValue)) {
            setDataPoints(prevPoints => {
              const newPoints = [
                ...prevPoints.slice(1), // Keep all previous points except the first one
                { value: humidityValue, timestamp: new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) }, // Add new data point with IST timestamp
              ];
              return newPoints;
            });
          }
